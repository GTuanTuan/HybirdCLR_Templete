...
namespace Templete
{
    public class ThirdController : MoveController
    {
        ...
        private void Awake()
        {
            ...
            CamCtrlState = Player.GetComponent<CamCtrlState>();
        }
        private void Update()
        {
            UpdateAxis();
        ...
        {
            if (CamCtrlState.mlock)
            {
                Rotate();
            }
            else
            {
                idle_Cam.GetComponent<Cinemachine.CinemachineFreeLook>().m_XAxis.m_MaxSpeed = 0f;
                idle_Cam.GetComponent<Cinemachine.CinemachineFreeLook>().m_YAxis.m_MaxSpeed = 0f;
            }
        }
        void Rotate()
        {
            idle_Cam.GetComponent<Cinemachine.CinemachineFreeLook>().m_XAxis.m_MaxSpeed = 400f;
            idle_Cam.GetComponent<Cinemachine.CinemachineFreeLook>().m_YAxis.m_MaxSpeed = 8f;
        ...
        void SetAnimData()
        {
            animData.bool_map[AnimController.Instance.MoveID] = TargetNotZero();
